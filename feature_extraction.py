...
class mnist_features:
        ...
        for k in range(len(hist['h'])):
            for i in range(0, 16-hist['h'][k]+1, 4):
                for j in range(0, 16-hist['w'][k]+1, 4):
                    patch = img[i:i + hist['h']
                                [k], j:j + hist['w'][k]].reshape(-1, )
                    hist1 = sum(patch < 0)
    ...
    def get_chain(self, img, chain):
        ...
        # obtain contours
        image, contours, hierarchy = cv2.findContours(img, cv2.RETR_EXTERNAL,
        ...
        for k, contour in enumerate(contours[1:]):
            for i, d in enumerate(directions):
                if (contour[0] == cur_c[0] + i_change[i]) and \
                   (contour[1] == cur_c[1] + j_change[i]):
                    p = get_pose(cur_c)
